...
function Contact() {
  ...
  const contactList = contactSection.contactList.map((contact, i) => {
    ...
    if(contact==8866245865){ 
      console.log(contact);
      return (
      <Col xs={12} key={i} >
        <h3 className="contact_section_contact" onClick={() => { window.open("tel:+918866245865");}}>
          <img
            draggable="false"
            className="emoji"
            alt="💠"
   ...
   else if(contact="harshil.sharaf1234@gmail.com"){

    return (
      <Col xs={12} key={i} >
        <h3 className="contact_section_contact" onClick={
